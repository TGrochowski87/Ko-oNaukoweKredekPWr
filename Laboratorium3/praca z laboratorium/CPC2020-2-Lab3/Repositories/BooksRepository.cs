...
namespace CPC2020_2_Lab3.Repositories
{
    ...
    public class BooksRepository: Repository, IBooksRepository
    {
        ...
        {
            string getBooksQuery = "SELECT Books.*, Authors.FirstName, Authors.LastName, Genres.Name FROM Books JOIN Authors ON Books.AuthorId = Authors.Id JOIN Genres ON Books.GenreId = Genres.Id; ";
        ...
        public void AddBook(string title, int yearOfPublish, float price, string genre, string authorFirstName, string authorLastName)
        {
            ...
            int genreId = (int)commandGetAuthorId.ExecuteScalar();
